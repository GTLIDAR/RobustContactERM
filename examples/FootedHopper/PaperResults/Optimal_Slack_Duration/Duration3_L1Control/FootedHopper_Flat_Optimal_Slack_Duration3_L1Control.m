function FootedHopper_Flat_Optimal_Slack_Duration3_L1Control()
...
% Load the warm start
here = pwd;
feasible = load([here, '/../../Feasible_Slack_Duration/Duration3/Slack_0E+00/FootedHopper_Flat_Feasible_Slack_Duration3.mat']);
...
% Cost weight matrix
R = 0.01*diag([1,1,1]);
% Pseudo-Huber Control Cost
p = sqrt(1 + u.^2);
dp = u./p;
g = sum(R*(p-1));

dg = [zeros(1,numel(t) + numel(x)), dp'*R];
