function FootedHopper_Flat_HeightERM_Slack_Duration3()
...
% Load the warm start
here = pwd;
feasible = load([here, '/../../Optimal_Slack_Duration/Duration3/FootedHopper_Flat_Optimal_Slack_Duration3.mat']);
...
guess.traj.slacks = feasible.soln.slacks(1:6,:);
...
optimOptions = setOptimizerOptions(optimOptions,'uncertainty_source',2, 'heightVariance',0.1, 'distanceCostMultiplier',10^4);
sigmas = [0.001, 0.01, 0.1,1];
for n = 1:length(sigmas)
    optimOptions = setOptimizerOptions(optimOptions, 'heightVariance', sigmas(n));
    
    soln = optimizePlant(plant,guess, optimOptions, snoptOptions);
    
    target = sprintf('ERM_HeightUncertainty_Sigma_%3E',sigmas(n));
