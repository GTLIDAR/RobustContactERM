function PushBlock_ERM_WeightTest()
...
optimOptions.display = true;
%% Solve trajectory optimization
% Generate a vector of friction coefficient variances
mult = [1e2, 1e3, 1e4, 1e5];
source = pwd;
% Tag
tag = [pwd, '/', name];
for n = 1:length(mult)
    % Change the folder
    target = ['multiplier_',sprintf('%3.0E',mult(n))];
    mkdir(target);
    cd(target);
    % Run the optimization
    optimOptions = setOptimizerOptions(optimOptions, 'frictionCostMultiplier',mult(n));
